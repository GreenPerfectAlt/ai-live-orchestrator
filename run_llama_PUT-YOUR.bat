...
set "MODEL_LABEL=PUT-YOUR_LABEL"
set "MODEL_PATH=PUT-YOUR_PATH-MODEL.GGUF"
set "MM_PROJ_PATH=PUT-YOUR_MODEL.GGUF"
set LLAMA_SERVER_EXE=PUT-YOUR_PATH-llama\llama-server.exe
set "LLAMA_MODEL=PUT-YOUR_NAME-MODEL"
...
set "LLAMA_FLAGS=-c 12000 --reasoning off --reasoning-budget 0 -ctk q8_0 -ctv q8_0 --temp 1.0 --top-k 0 --top-p 1.0 --min-p 0.05 --typical 1.00 --dynatemp-range 0.00 --mirostat 0 --spec-type ngram-simple --spec-draft-n-max 3 --ctx-checkpoints 128 -n -1 -ngl -1 --prio 3 -fa on -fit on -t 6 --threads-batch 6 --parallel 1 --keep 1 --port 8080 -b 512 -ub 512 --jinja --no-mmproj-offload"
...
if not defined LLAMA_SERVER_EXE (
  ...
  if not defined LLAMA_SERVER_EXE if exist "%PROJECT_ROOT%llama.cpp\build\bin\Release\llama-server.exe" set "LLAMA_SERVER_EXE=%PROJECT_ROOT%llama.cpp\build\bin\Release\llama-server.exe"
  if not defined LLAMA_SERVER_EXE if exist "%PROJECT_ROOT%llama.cpp\build\bin\llama-server.exe" set "LLAMA_SERVER_EXE=%PROJECT_ROOT%llama.cpp\build\bin\llama-server.exe"
  if not defined LLAMA_SERVER_EXE set "LLAMA_SERVER_EXE=llama-server.exe"
)
