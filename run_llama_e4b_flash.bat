...
set "LAUNCHER_NAME=flash/run_llama_e4b.bat"
set "MODEL_LABEL=Gemma-4-E4B QAT llama.cpp"
set "MODEL_PATH=%PROJECT_ROOT%models\gemma-4-E4B-it-qat-UD-Q4_K_XL.gguf"
set "MM_PROJ_PATH=%PROJECT_ROOT%models\mmproj-F16-gemma-4-E4B-it-qat.gguf"
if not exist "%MM_PROJ_PATH%" if exist "%PROJECT_ROOT%models\mmproj-F16-gemma-4-E4B-it-qat" set "MM_PROJ_PATH=%PROJECT_ROOT%models\mmproj-F16-gemma-4-E4B-it-qat"
...
set "LLAMA_MODEL=gemma-4-E4B-it-qat"
...
if not exist "%MODEL_PATH%" (
  ...
  echo %MODEL_PATH%
  echo.
  echo Put gemma-4-E4B-it-qat-UD-Q4_K_XL.gguf into the models folder.
  pause
  exit /b 1
)
...
if not exist "%MM_PROJ_PATH%" (
  ...
  echo %MM_PROJ_PATH%
  echo.
  echo Put mmproj-F16-gemma-4-E4B-it-qat.gguf into the models folder.
  pause
  exit /b 1
)
