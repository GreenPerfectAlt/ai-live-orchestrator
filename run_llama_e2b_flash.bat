...
:: Получаем путь к папке скрипта в формате 8.3 (без проблем с кириллицей и пробелами)
...
:: --- ПОИСК PYTHON ---
if exist "C:\AI\Python\Python311\python.exe" (
...
:: --- ПУТИ К МОДЕЛЯМ И ИСПОЛНЯЕМЫМ ФАЙЛАМ ---
...
:: Если путь указан как папка, авто-дописываем имя файла
if exist "%LLAMA_SERVER_EXE%\llama-server.exe" (
  set "LLAMA_SERVER_EXE=%LLAMA_SERVER_EXE%\llama-server.exe"
)
...
:: --- НАСТРОЙКИ ПОТОКОВ И СЕРВЕРА LLAMA ---
set "LLAMA_THREADS=6"
...
if not exist "%MODEL_PATH%" (
  echo [ERROR] GGUF model not found: %MODEL_PATH%
  pause
  exit /b 1
)

if not exist "%MM_PROJ_PATH%" (
  echo [ERROR] mmproj file not found: %MM_PROJ_PATH%
  pause
  exit /b 1
)
...
if not exist "%LLAMA_SERVER_EXE%" (
  ...
  if errorlevel 1 (
    echo [ERROR] llama-server.exe not found at: %LLAMA_SERVER_EXE%
    pause
    exit /b 1
  )
...
:: 1) PyTorch CPU ставится ПЕРВЫМ и отдельно (CPU-wheel с pytorch.org), чтобы
::    faster-whisper / silero не потянули тяжёлую CUDA-сборку torch через pip.
...
:: 2) Остальное — из requirements.txt (fastapi, uvicorn, websockets, numpy,
::    supertonic, faster-whisper...). faster-whisper поставится отсюда,
::    отдельный блок про него НЕ нужен.
...
if errorlevel 1 (
echo [WARN] pip install -r requirements.txt failed. Falling back to core deps (native mode will work without chat text)...
"%PORTABLE_PYTHON_EXE%" -m pip install fastapi "uvicorn[standard]" websockets "numpy>=2.0.0" "supertonic>=1.3.1" --no-warn-script-location
)
)

:: --- ЗАПУСК CЕРВЕРА И ПРИЛОЖЕНИЯ ---
...
echo ===================================================
echo [Parlor] Starting Gemma-4-E2B QAT
