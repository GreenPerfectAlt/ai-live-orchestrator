...
rem Required for camera/screen/image/PDF/video context and native audio.
...
rem Context size. Lower if RAM is tight. Safe: 4096 / 8192. Heavy: 12000.
...
rem ===================================================
rem USER CONFIG - REASONING (thinking)
rem ===================================================
rem LLAMA_REASONING: off = no thinking (fast, default). auto = model may think.
rem To enable "regenerate with reasoning", set LLAMA_REASONING=auto and a budget.
rem LLAMA_REASONING_BUDGET: max thinking tokens. 0 = unlimited when reasoning!=off.
set "LLAMA_REASONING=off"
set "LLAMA_REASONING_BUDGET=0"
rem ===================================================
...
set "TTS_THREADS=2"
rem ===================================================
rem USER CONFIG - STT (server-side transcription for native audio)
rem ===================================================
rem Used when the frontend sends native audio (Gemma mic input mode = Native).
rem faster-whisper transcribes your speech into the chat bubble.
...
rem Reasoning: off disables thinking; auto enables it (budget controls length).
set "LLAMA_FLAGS=%LLAMA_FLAGS% --reasoning %LLAMA_REASONING% --reasoning-budget %LLAMA_REASONING_BUDGET%"
...
rem PyTorch CPU first (CPU wheel, not CUDA) - needed for Silero TTS.
...
rem Everything else from requirements.txt (fastapi, uvicorn, websockets, numpy,
rem requests, supertonic, faster-whisper...).
python -c "import fastapi,uvicorn,numpy,websockets,faster_whisper" >nul 2>nul
if errorlevel 1 (
echo [SETUP] Installing backend dependencies from requirements.txt ...
python -m pip install -U pip setuptools wheel
if errorlevel 1 goto pip_failed
python -m pip install -r "%PROJECT_ROOT%requirements.txt" --no-warn-script-location
if errorlevel 1 (
echo [WARN] requirements.txt failed. Installing core deps without faster-whisper...
python -m pip install fastapi "uvicorn[standard]" websockets "numpy>=2.0.0" requests "supertonic>=1.3.1" --no-warn-script-location
if errorlevel 1 goto pip_failed
)
)
rem Final check of critical packages.
...
echo Reasoning:    %LLAMA_REASONING% (budget %LLAMA_REASONING_BUDGET%)
echo App URL:      http://127.0.0.1:8000
echo TTS:          %TTS_ENGINE% / %TTS_LANG% / %TTS_VOICE%
echo STT:          %STT_ENGINE% / %STT_MODEL% / %STT_LANG%
echo ===================================================
python server.py
pause
exit /b 0
