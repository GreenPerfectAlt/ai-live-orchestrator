...
set "LAUNCHER_NAME=flash/run_llama_12B.bat"
set "MODEL_LABEL=Gemma-4-12B QAT llama.cpp"
set "MODEL_PATH=%PROJECT_ROOT%models\gemma-4-12B-it-qat-UD-Q4_K_XL.gguf"
set "MM_PROJ_PATH=%PROJECT_ROOT%models\mmproj-F16-gemma-4-12B-it-qat.gguf"
if not exist "%MM_PROJ_PATH%" if exist "%PROJECT_ROOT%models\mmproj-F16-gemma-4-12B-it-qat" set "MM_PROJ_PATH=%PROJECT_ROOT%models\mmproj-F16-gemma-4-12B-it-qat"

rem Flash-friendly: prefer llama-server next to the project, then C:\AI, then PATH.
...
if not defined LLAMA_SERVER_EXE if exist "%PROJECT_ROOT%llama\llama-server.exe" set "LLAMA_SERVER_EXE=%PROJECT_ROOT%llama-server.exe"
...
if not defined LLAMA_SERVER_EXE if exist "C:\AI\llama\llama-server.exe" set "LLAMA_SERVER_EXE=%PROJECT_ROOT%llama-server.exe"
set "LLAMA_MODEL=gemma-4-12B-it-qat"
...
if not exist "%MODEL_PATH%" (
  ...
  echo %MODEL_PATH%
  echo.
  echo Put gemma-4-12B-it-qat-UD-Q4_K_XL.gguf into the models folder.
  pause
  exit /b 1
)
...
if not exist "%MM_PROJ_PATH%" (
  ...
  echo %MM_PROJ_PATH%
  echo.
  echo Put mmproj-F16-gemma-4-12B-it-qat.gguf into the models folder.
  pause
  exit /b 1
)
...
:llama_exe_ok
rem Flash-friendly: do NOT use a venv copied from another drive.
rem The Python environment is kept on this Windows PC, not on the flash drive.
set "PARLOR_VENV_ROOT=%LOCALAPPDATA%\ParlorJarvis"
set "PARLOR_VENV_DIR=%PARLOR_VENV_ROOT%\venv-llamacpp"
if not exist "%PARLOR_VENV_DIR%\Scripts\python.exe" (
  call :create_venv
  if errorlevel 1 (
    pause
    exit /b 1
...
call "%PARLOR_VENV_DIR%\Scripts\activate.bat" || (
  echo [ERROR] Cannot activate local Parlor venv:
...
if errorlevel 1 (
  echo [START] Starting llama-server E4B on 127.0.0.1:8080 ...
  start "llama-server E4B" cmd /k ""%LLAMA_SERVER_EXE%" -m "%MODEL_PATH%" --mmproj "%MM_PROJ_PATH%" --alias "%LLAMA_MODEL%" --host 127.0.0.1 %LLAMA_FLAGS%"
...
echo ===================================================
echo [Parlor llama.cpp] Starting Gemma-4-E4B QAT
...
echo Python venv:  %PARLOR_VENV_DIR%
echo Streaming:    ON ^(text_delta + low-latency sentence TTS^). Camera is not sent unless asked.
echo ===================================================
...
:create_venv
echo [SETUP] Local Python environment not found on this PC.
echo [SETUP] Creating local venv here:
echo %PARLOR_VENV_DIR%
if not exist "%PARLOR_VENV_ROOT%" mkdir "%PARLOR_VENV_ROOT%"

where py >nul 2>nul
if not errorlevel 1 (
  py -3.12 -m venv "%PARLOR_VENV_DIR%" >nul 2>nul
  if exist "%PARLOR_VENV_DIR%\Scripts\python.exe" exit /b 0
  py -3.11 -m venv "%PARLOR_VENV_DIR%" >nul 2>nul
  if exist "%PARLOR_VENV_DIR%\Scripts\python.exe" exit /b 0
)

where python >nul 2>nul
if not errorlevel 1 (
  python -m venv "%PARLOR_VENV_DIR%"
  if exist "%PARLOR_VENV_DIR%\Scripts\python.exe" exit /b 0
)

echo.
echo [ERROR] Python 3.11 or 3.12 was not found on this computer.
echo This flash build does NOT include portable Python.
echo Install normal Python on the PC and enable "Add python.exe to PATH",
echo or install Python from Microsoft Store / python.org.
exit /b 1
